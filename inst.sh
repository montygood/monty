...
arch_chroot "pacman -S git --noconfirm" && arch_chroot "sudo -u ${USERNAME} git clone https://aur.archlinux.org/trizen.git" && arch_chroot "cd trizen" && arch_chroot "sudo -u ${USERNAME} makepkg -si --noconfirm"
	...
	arch_chroot "su - ${USERNAME} -c 'trizen -S gksu --noinstall --noconfirm'" && arch_chroot "pacman -U /tmp/*/gksu/*.pkg.tar.xz --noconfirm'"
	...
	#WiFi
	[[ $(lspci | egrep Wireless | egrep Broadcom) != "" ]] && arch_chroot "su - ${USERNAME} -c 'trizen -S broadcom-wl --noinstall --noconfirm'" && arch_chroot "pacman -U /tmp/*/broadcom-wl/*.pkg.tar.xz --noconfirm'"
	...
	#Oberfäche
	arch_strap "cinnamon cinnamon-translations nemo-fileroller nemo-preview gnome-terminal gnome-screenshot eog gnome-calculator networkmanager"
	#Anmeldescreen
	...
	#pamac
	arch_chroot "su - ${USERNAME} -c 'trizen -S pamac-aur --noinstall --noconfirm'" && arch_chroot "pacman -U /tmp/*/pamac-aur/*.pkg.tar.xz --noconfirm'"
	...
	#Skype
	arch_chroot "su - ${USERNAME} -c 'trizen -S skypeforlinux-preview-bin --noinstall --noconfirm'" && arch_chroot "pacman -U /tmp/*/skypeforlinux-preview-bin/*.pkg.tar.xz --noconfirm'"
	#Teamviewer
	arch_chroot "su - ${USERNAME} -c 'trizen -S teamviewer --noinstall --noconfirm'" && arch_chroot "pacman -U /tmp/*/teamviewer/*.pkg.tar.xz --noconfirm'"
	arch_chroot "systemctl enable teamviewerd"
	#Fingerprint
	if [[ $(lsusb | grep Fingerprint) != "" ]]; then		
		arch_chroot "su - ${USERNAME} -c 'trizen -S fingerprint-gui --noinstall --noconfirm'" && arch_chroot "pacman -U /tmp/*/fingerprint-gui/*.pkg.tar.xz --noconfirm'"
	...
	arch_chroot "trizen -Syu --noconfirm"
	#Error
	#check_error
