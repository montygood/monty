...
_mirrors() {
	if ! (</etc/pacman.d/mirrorlist2 grep "rankmirrors" &>/dev/null) then
		...
		pacman-key --refresh-keys 2>.err & pid=$! pri=0.8 msg="Spiegelserver-Schlüssel werden geupdatet..." load
		pacman -Syy
	fi
	_part
}
_part() {
    ...
    if [[ $SYSTEM == "BIOS" ]]; then
		...
		echo j | mkfs.ext4 -L arch /dev/${IDEV}2 >/dev/null
	...
	if [[ $SYSTEM == "UEFI" ]]; then
		...
		echo j | mkfs.vfat -F32 -L boot /dev/${IDEV}1 >/dev/null
		echo j | mkfs.ext4 -L arch /dev/${IDEV}2 >/dev/null
		mount /dev/${IDEV}2 /mnt
		mkdir -p /mnt/boot
...
_mediaelch() {
	if [[ $MPC == "YES" ]]; then		
		...
		echo "[Directories]" >> /mnt/home/${USER}/.config/kvibes/MediaElch.conf
