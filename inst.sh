...
function start {
	...
	echo LANGUAGE=de_CH >> /mnt/etc/locale.conf
...
EOF
	...
	#pamac
	mv pamac-aur-any.pkg.tar.xz /mnt && arch-chroot /mnt /bin/bash -c "pacman -U pamac-aur-any.pkg.tar.xz --needed --noconfirm" && rm /mnt/pamac-aur-any.pkg.tar.xz
#	arch-chroot /mnt /bin/bash -c "echo $RPASSWD | su - ${USERNAME} -c 'trizen -S pamac-aur --noconfirm'"
	...
	#Zusatz
	[[ $GIMP == "YES" ]] && arch-chroot /mnt "pacman -S --needed --noconfirm gimp gimp-help-de gimp-plugin-gmic gimp-plugin-fblur"
	[[ $OFFICE == "YES" ]] && arch-chroot /mnt "pacman -S --needed --noconfirm libreoffice-fresh libreoffice-fresh-de hunspell-de aspell-de"
	[[ $WINE == "YES" ]] && arch-chroot /mnt "pacman -S --needed --noconfirm wine wine_gecko wine-mono winetricks lib32-libxcomposite lib32-libglvnd"
	[[ $TEAM == "YES" ]] && mv anydesk-any.pkg.tar.xz /mnt && arch-chroot /mnt /bin/bash -c "pacman -U anydesk-any.pkg.tar.xz --needed --noconfirm" && rm /mnt/anydesk-any.pkg.tar.xz
#	[[ $TEAM == "YES" ]] && arch-chroot /mnt "echo $RPASSWD | su - ${USERNAME} -c 'trizen -S anydesk --noconfirm'"
	if [[ $FBOT == "YES" ]]; then		
		arch-chroot /mnt "pacman -S --needed --noconfirm java-openjfx libmediainfo"
	...
	#Treiber
	[[ $(lspci | egrep Wireless | egrep Broadcom) != "" ]] && arch-chroot /mnt "pacman -S --needed --noconfirm broadcom-wl"
	[[ $(dmesg | egrep Bluetooth) != "" ]] && arch-chroot /mnt "pacman -S --needed --noconfirm blueberry bluez bluez-firmware pulseaudio-bluetooth" && arch-chroot /mnt /bin/bash -c "systemctl enable bluetooth.service"
	[[ $(dmesg | egrep Touchpad) != "" ]] && arch-chroot /mnt "pacman -S --needed --noconfirm xf86-input-libinput"
	[[ $HD_SD == "SSD" ]] && arch-chroot /mnt "systemctl enable fstrim && systemctl enable fstrim.timer"
...
https://aur.archlinux.org/cgit/aur.git/snapshot/fprintd-vfs_proprietary.tar.gz
	...
	#Mintstick
	mv mintstick-any.pkg.tar.xz /mnt && arch-chroot /mnt /bin/bash -c "pacman -U mintstick-any.pkg.tar.xz --needed --noconfirm" && rm /mnt/mintstick-any.pkg.tar.xz
#	arch-chroot /mnt /bin/bash -c "su - ${USERNAME} -c 'trizen -S mintstick --noconfirm'"
	#myup
	echo '#!/bin/sh' >> /mnt/bin/myup
	echo "sudo pacman -Syu --noconfirm --needed --noprogressbar" >> /mnt/bin/myup
	echo "trizen -Syu --noconfirm --needed --noprogressbar" >> /mnt/bin/myup
	echo "sudo pacman -Rns --noconfirm --needed --noprogressbar $(sudo pacman -Qtdq --noconfirm --needed --noprogressbar)" >> /mnt/bin/myup
	echo "sudo pacman -Scc --noconfirm --needed --noprogressbar" >> /mnt/bin/myup
...
EOF
	...
	#finish
	mv monty.tar.gz /mnt && arch-chroot /mnt "tar xvf monty.tar.gz" && rm /mnt/monty.tar.gz
